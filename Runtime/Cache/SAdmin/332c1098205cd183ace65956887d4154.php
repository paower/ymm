...
<html lang="en">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane" id="tab-shipping">
            <table class="table table-bordered">
                ...
                <tr>
                    <td>货运方式</td>
                    <td><?php echo ((isset($order['kd_name']) && ($order['kd_name'] !== ""))?($order['kd_name']):"无"); ?></td>
                </tr>
 ...
 $productList = M("order_detail")->where(array('order_id'=>$order['order_id']))->select(); foreach($productList as $key=>$product){ ?>
                <tr>
                    <td class="left"><?php echo ($product['com_id']); ?></td>
                    <td class="left"><?php if(($product['shangjia']) != ""): ?>商家(<?php echo M('gerenshangpu')->where(array('id'=>$product['shangjia']))->getfield('shop_name'); ?>)订单<?php else: ?>系统订单<?php endif; ?></td>
        ...
        <div id="tab-fahuo" class="tab-pane">
            <form action="<?php echo U('/Order/give');?>" method="post">
                ...
                <tr>
                    <td>货运方式</td>
                    <td><input type="text" value="<?php echo ($order['kd_name']); ?>" size="15" name="kd_type" placeholder="快递类型"/></td>
                </tr>
                <tr>
                    <td>快递单号</td>
                    <td><input type="text" value="<?php echo ($order['kd_no']); ?>" size="60" name="kd_no" placeholder="快递单号"/></td>
                </tr>
                <tr>
