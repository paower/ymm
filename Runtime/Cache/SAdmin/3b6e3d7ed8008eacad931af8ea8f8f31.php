...
<html lang="en">
  ...
  <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover search-form">
                    <thead>
                        <th><input name="name" type="text" placeholder="输入商铺名称" value="<?php echo I('name');?>" /></th>
                        <th>    				
                            <select name="shop_cate">
                        ...
                        <th>
                            <a class="btn btn-primary" href="javascript:;" id="search" url="<?php echo U('Goods/ggshop');?>">查询</a>
                        </th>
                    </thead>
                </table>	
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <?php if(is_array($ggshop)): $i = 0; $__LIST__ = $ggshop;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $id['member_id']=$v['userid']; $dengji=M('member')->where($id)->field('member_grade')->find(); ?> 
                        <tr>
							...
							<td>
                                <?php if($v['shop_info'] == 1): ?><a class="btn btn-xs btn-info" href='<?php echo U("goods/shop_info",array("id"=>$v["id"],"shop_info"=>0));?>'> 已启用</a>
                                <?php else: ?>
                                <a class="btn btn-xs btn-info" style="color:#cf1515 !important;" href='<?php echo U("goods/shop_info",array("id"=>$v["id"],"shop_info"=>1));?>'> 已禁用</a><?php endif; ?>
                            </td>
