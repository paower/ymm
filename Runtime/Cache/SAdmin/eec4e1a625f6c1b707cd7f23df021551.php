...
<div class="padding border-bottom">  
  <a class="btn btn-primary" href='<?php echo U("Goods/addCate");?>'><span class="icon-plus-square-o"></span> 添加分类</a>
  </div>	
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						<?php if(is_array($oneCate)): $i = 0; $__LIST__ = $oneCate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one): $mod = ($i % 2 );++$i; if($one['pid'] == 0): ?><tr>
									...
									<td class="listorder"><input type="text" size="3" name="listorder" data-id="<?php echo ($one['id']); ?>" value="<?php echo ($one['sort']); ?>"></td>
								...
								<?php if(is_array($twoCate)): $i = 0; $__LIST__ = $twoCate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$two): $mod = ($i % 2 );++$i; if($two['pid'] == $one['id']): ?><tr>
												...
												<td class="listorder"><input type="text" size="3" data-id="<?php echo ($two['id']); ?>"  name="listorder" value="<?php echo ($two['sort']); ?>"></td>
												<td></td>
											...
											<?php if(is_array($threeCate)): $i = 0; $__LIST__ = $threeCate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$three): $mod = ($i % 2 );++$i; if($three['pid'] == $two['id']): ?><tr>
														...
														<td class="listorder"><input type="text" data-id="<?php echo ($three['id']); ?>" size="3" name="listorder" value="<?php echo ($three['sort']); ?>"></td>
														<td></td>
														<td></td>
...
<script>
	$(".listorder input").blur(function(){
		...
		$.post('<?php echo U("Goods/catesort");?>',{'id':id,'sort':sort},function(res){
			if(res.code==1){
				location.href="<?php echo U('Goods/cate');?>";
			}else{
				alert(res.msg);
			}
		},'json');
