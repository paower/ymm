...
class ComplaintController extends AdminController
{
    ...
    public function edit(){
        ...
        if(IS_POST){
            $id = I('id');
            $reply = I('reply');
            $status = $letter->where(array('id'=>$id))->getField('status');
            if($reply != ''&&$status == 0){
                $letter->where(array('id'=>$id))->setField('status',1);
            }
            $res = $letter->where(array('id'=>$id))->setField('imgs',$reply);
            dump($letter->getLastSql());
            if($res !== false){
                echo "<meta charset='utf-8'><script>alert('回复成功');location.href='".U('complaint/index')."'</script>";
                die;
            }else{
                // echo "<meta charset='utf-8'><script>alert('回复失败');location.href='".U('complaint/index')."'</script>";
                die;
