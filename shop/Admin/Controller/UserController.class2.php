...
class UserController extends AdminController
{
		...
		public function userrechar_examine(){
			...
			if($res !== false && $res2 !== false){
				...
				$userdata = M('user')->where(array('userid'=>$user['uid']))->find();
				//直推
				if($userdata['pid'] != 0){
					$reward = $user['money'] * $rechar_firstReward;
					M('store')->where(array('uid'=>$userdata['pid']))->setInc('recharge_num',$reward * $reward_to_rechar);
					M('store')->where(array('uid'=>$userdata['pid']))->setInc('cangku_num',$reward * $reward_to_shopEta);
				}
				//间推
				if($userdata['gid'] != 0){
					$reward = $user['money'] * $rechar_secondReward;
					M('store')->where(array('uid'=>$userdata['gid']))->setInc('recharge_num',$reward * $reward_to_rechar);
					M('store')->where(array('uid'=>$userdata['gid']))->setInc('cangku_num',$reward * $reward_to_shopEta);
				}
				$this->success('审核成功');
			}else{
				$this->error('审核失败');
		...
		*/
		...
		public function record(){
				...
				$page=$p->show();
				$data_list = $table
						->field('u.userid,u.username,u.account,u.mobile,t.get_nums,t.pay_id,t.get_time,t.get_type')
						->where($map)
						->order($order_str)
						->select();
