...
class TradingController extends CommonController {
    ...
    public function SellCentr(){
        $id = session('userid');
        $tixian = M('user')->where("userid = $id")->getField('tixian');
        //判断提现是否关闭
         // $close=is_close_tixian();
         // if($close['value']==0){
         if($tixian==0){
             success_alert('提现暂时关闭',U('index/index'));
         }

        //是否有设置默认银行卡
        $uid = session('userid');
        $cid = trim(I('cid'));
		$money = M('store')->where(array('uid'=>$uid))->getField('cangku_num');
		$money2 = M('store')->where(array('uid'=>$uid))->getField('cangku2_num');
        if(empty($cid)){
        ...
        if(IS_AJAX){
            ...
            if($res_Add){
                ...
                //自己是否有足够余额
                $is_enough = M('store')->where(array('uid'=>$uid))->getField('cangku2_num');
                ...
                $transfer_sell = $sellnums * $cash2_ratio;
                ...
                if($res_Add){
                    // $sellnums = $sellnums + 100;
                    $doDec = M('store')->where(array('uid'=>$uid))->setDec('cangku2_num',$sellnums);
 ...
 public function quxiao_order(){
    ...
    if($type==0){//卖出单，删除订单
            ...
            if($res1){
                ...
                //获取消除的比例
                $transfer_ratio = M('config')->where(array('name'=>'transfer_ratio'))->getField('value');
                $sellnums = $sellnums/$transfer_ratio;
    ...
    public function Dosells(){
        if(IS_AJAX){
            ...
            $user_info = $user_object->Trans($minepwd['account'], $pwd);
                ...
                //是否绑定银行卡
                $id_setbank = M('ubanks')->where(array('user_id'=>$uid,'is_default'=>1))->count('1');
				//是否绑定微信
				$id_setwx = M('uwx')->where(array('userid'=>$uid,'is_default' => 1))->count('1');
				//是否绑定支付宝
				$id_setalipay = M('ualipay')->where(array('userid'=>$uid,'is_default'=>1))->count('1');
                if($id_setbank < 1 && $id_setwx < 1 && $id_setalipay < 1){
                    $id_setcards = M('ubanks')->where(array('user_id'=>$uid))->limit(1)->find();
                }else{

					if($id_setbank == 1){
						$id_setcards = M('ubanks')->where(array('user_id'=>$uid,'is_default'=>1))->find();
					}elseif($id_setwx == 1){
						$id_setcards = M('uwx')->where(array('userid'=>$uid,'is_default'=>1))->find();
					}elseif($id_setalipay == 1){
						$id_setcards = M('ualipay')->where(array('userid'=>$uid,'is_default'=>1))->find();
					}
				}
				...
				if($id_setbank == 1){
					...
					$arr['card_id'] = $id_setcards['id'];
					$arr['card_type'] = 'alipay';
				}
                $res_Buy = M('trans')->where(array('id'=>$trid))->save($arr);
                if($res_Buy){
                    ajaxReturn('卖出成功',1);
                }
        }
