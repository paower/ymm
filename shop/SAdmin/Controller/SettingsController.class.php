...
class SettingsController extends CommonController
{
	...
	{
		$buy_firstReward =  M('config')->where(array('name'=>'buy_firstReward'))->getField('value');
		$buy_secondReward =  M('config')->where(array('name'=>'buy_secondReward'))->getField('value');
		$buyReward_to_rechar =  M('config')->where(array('name'=>'buyReward_to_rechar'))->getField('value');
		$buyReward_to_shopEta =  M('config')->where(array('name'=>'buyReward_to_shopEta'))->getField('value');
		$sell_firstReward =  M('config')->where(array('name'=>'sell_firstReward'))->getField('value');
		$sell_secondReward =  M('config')->where(array('name'=>'sell_secondReward'))->getField('value');
		$sellReward_to_rechar =  M('config')->where(array('name'=>'sellReward_to_rechar'))->getField('value');
		$sellReward_to_shopEta =  M('config')->where(array('name'=>'sellReward_to_shopEta'))->getField('value');
		$this->assign(array(
			'buy_firstReward'=>$buy_firstReward,
			'buy_secondReward'=>$buy_secondReward,
			'buyReward_to_rechar'=>$buyReward_to_rechar,
			'buyReward_to_shopEta'=>$buyReward_to_shopEta,
			'sell_firstReward'=>$sell_firstReward,
			'sell_secondReward'=>$sell_secondReward,
			'sellReward_to_rechar'=>$sellReward_to_rechar,
			'sellReward_to_shopEta'=>$sellReward_to_shopEta
		));
		$this->breadcrumb1 = "商城";
		$this->breadcrumb2 = "奖励管理";
		$this->display();
	}
	//修改奖励
	public function saveReward()
	{
		if(IS_POST){
			$reward = I('post.');
			if ((float)$reward['buyReward_to_rechar'] + (float)$reward['buyReward_to_shopEta'] != 100 || (float)$reward['sellReward_to_rechar'] + (float)$reward['sellReward_to_shopEta'] != 100) {
				$this->error('奖励进入比例为百分百');
				exit;
			}
			...
			if ($reward && is_array($reward)) {
				...
				foreach ($reward as $name => $value) {
					$map = array('name' => $name);
					$c->where($map)->setField('value', (float)$value / 100);
				}
	...
	/*轮播图*/
	public function sshop_banner()
	{
		$this->breadcrumb2 = '轮播图';
		$nzbill = M('sshop_banner');
		$count = $nzbill->count();
		$Page = new \Think\Page($count, 20);
		$show = $Page->show();
		$list = $nzbill->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
	...
	function del11()
	{
		$id = I('id');
		if ($id != '') {
			$res = M('sshop_banner')->where(array('id' => $id))->delete();
			S('DB_CONFIG_DATA', null);
			$this->success('删除成功！');
		}
	}
	public function add(){
		if(IS_POST){
		 $upload = new \Think\Upload();// 实例化上传类
			...
			// 上传文件 
			$info   =   $upload->upload();
			$path['pic'] = "/Uploads/".$info['file0']['savepath'].$info['file0']['savename'];
			M('sshop_banner')->add($path);
			if(!$info) {// 上传错误提示错误信息
			    $this->error($upload->getError());
			}else{// 上传成功
			    $this->success('上传成功！',U('settings/sshop_banner'));
			}
		}
		$this->display();
	...
	public function edit(){
		$id = I('id');
		$sshop_banner = M('sshop_banner')->where(array('id'=>$id))->find();
		if(IS_POST){
		 $upload = new \Think\Upload();// 实例化上传类
			$upload->maxSize   =     3145728 ;// 设置附件上传大小
			$upload->exts      =     array('jpg', 'gif', 'png', 'jpeg');// 设置附件上传类型
			...
			M('sshop_banner')->where(array('id'=>$id))->save($path);
			if(!$info) {// 上传错误提示错误信息
			    $this->error($upload->getError());
			}else{// 上传成功
			    $this->success('修改成功！',U('settings/sshop_banner'));
			}
		}
	...
	public function dltwen(){
		...
		if($id=4 || $id=5 || $id=6 || $id=13){
           echo '<script>alert("帮助中心、MXC介绍、商城介绍、联系客服 根据需求删除!"); window.history.back(-1); </script>';
		}else{
	...
	public function Savewentype()
	{
		...
		if ($id != '') {
			if(3<=$id && $id<=6){
				echo '<script>alert("帮助中心、MXC介绍、商城介绍、联系客服 根据需求修改!"); window.history.back(-1); </script>';
			}else{
				$res = $configsAll->where(array('id' => $id))->save($data);
	...
	public function Savenotice()
	{
		...
		S('DB_CONFIG_DATA', null);
		$this->success('保存成功！',U('settings/notice'));
	}
