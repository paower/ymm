...
class GoodsModel extends Model
{
	...
	//	}
		...
		$sql .= ' order by pd.ctime desc LIMIT ' . $Page->firstRow . ',' . $Page->listRows;
...
/*   var_dump($zong_s);die;*/
		...
		$goods['is_shangjia'] = $data['shangjia'];
		$goods['is_duobao'] = 1;

		$goods['set_meal'] = $data['set_meal']; //是否为套餐

		$goods['is_tuijian'] = $data['tuijian'];

		$data['zhifu_pay'] = implode(',',$data['zhifu_pay']);
		if(!$data['zhifu_pay']){
			return array(
				'status' => 'fail',
				'message' => '请勾选支付方式',
		...
		$goods['zhifu_pay']=$data['zhifu_pay'];

		$goods['is_teshu']=$data['is_teshu'];
		//消费积分使用比例
		...
		//返还余额
		$goods['return_balance'] = $data['return_balance'];
		
		$info['type'] = $data['cate'];
		$info['price'] = $data['price'];
		$info['kucun'] = $data['kucun'];
		$count = count($info['type']);

		M('product_parameter')->where("gid=$id")->delete();
		for ($i=0; $i < $count ; $i++) { 
			$date['gid'] = $id;
			$date['type'] = $info['type'][$i];
			$date['price'] = $info['price'][$i];
			$date['kucun'] = $info['kucun'][$i];
			$date['jifen_nums'] = $data['jifen_nums'];
			M('product_parameter')->add($date);
		}
	...
	function add_Goods($data)
	{
		...
		$goods['zhifu_pay'] = implode(',',$goods['zhifu_pay']);
		if(!$data['zhifu_pay']){
			return array(
				'status' => 'fail',
				'message' => '请勾选支付方式',
...
function add_dianpu($data)
	{
		...
		$goods['lng'] = $data['lng'];
		$goods['lat'] = $data['lat'];
		$goods['shop_phone'] = $data['shop_phone'];
