...
class OrderModel{
	...
	public function show_order_page($search){
		...
		$sql="SELECT o.order_id,o.order_no,o.uid,m.username,m.mobile,o.status,o.time,o.buy_price,o.buy_name,o.buy_phone,o.seluid FROM "
		.C('DB_PREFIX').'order o,'.C('DB_PREFIX').'user m WHERE o.uid=m.userid ';
		...
		if(isset($search['user_phone'])){
			$sql.=" and m.mobile like '%".$search['user_phone']."%'";
		}
	...
	public function order_info($id){
		//订单详情
		$order_sql="SELECT o.*,m.uname,m.phone FROM "
		.C('DB_PREFIX').'order o,'.C('DB_PREFIX').'member m WHERE m.member_id = o.uid AND  o.order_id='.$id;
