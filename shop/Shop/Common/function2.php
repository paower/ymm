...
function file_uploading($path_old = null)
{
	...
	// }
		if($info['pic']){
			$res['pic']['status']='success';
			$res['pic']['res']='/Uploads/image/product/'.$info['pic']['savepath'].$info['pic']['savename'];
			$res['pic']['name']=$info['pic']['key'];
		}
		if($info['pic1']){
			$res['pic1']['status']='success';
			$res['pic1']['res']='/Uploads/image/product/'.$info['pic1']['savepath'].$info['pic1']['savename'];
			$res['pic1']['name']=$info['pic1']['key'];
		}
		if($info['pic2']){
			$res['pic2']['status']='success';
			$res['pic2']['res']='/Uploads/image/product/'.$info['pic2']['savepath'].$info['pic2']['savename'];
			$res['pic2']['name']=$info['pic2']['key'];
		}
		if($info['pic3']){
			$res['pic3']['status']='success';
			$res['pic3']['res']='/Uploads/image/product/'.$info['pic3']['savepath'].$info['pic3']['savename'];
			$res['pic3']['name']=$info['pic3']['key'];
		}
		if($info['pic4']){
			$res['pic4']['status']='success';
			$res['pic4']['res']='/Uploads/image/product/'.$info['pic4']['savepath'].$info['pic4']['savename'];
			$res['pic4']['name']=$info['pic4']['key'];
		}
...
function moreimg_uploads($path_old = null)
{
	...
	$upload = new \Think\Upload();// 实例化上传类
	$upload->maxSize = 61457280;// 设置附件上传大小
	$upload->exts = array('jpg', 'gif', 'png', 'jpeg');// 设置附件上传类型
	$upload->rootPath = $images_path; // 设置上传根目录    // 上传文件
	$upload->savePath = ''; // 设置上传子目录    // 上传文件
 ...
 * $id 购买商品用户id
 */
function AddUserLevel($pid,$gid,$ggid,$id){
	$where['uid']=$pid;
	//直推人数
	$table=M('user_level');
	$children_num=$table->where($where)->getField('children_num');
	$table2=M('reward');
	$jl_num=$table2->where('id=1')->getfield('jl_num');
	$zg_num=$table2->where('id=1')->getfield('zg_num');

	$vip_grade=M('user')->where(array('id'=>$pid,'vip_grade'=>array('gt',0)))->getField('vip_grade');

	if($children_num>=$zg_num){
		$level=2;//主管
		//推荐人升级为主管
		if($vip_grade<$level && !empty($vip_grade)){
			M('user')->where(array('id'=>$pid))->setField('vip_grade',$level);
			
			//上上级升级为经理
			$zhuguan_num=M('user')->where(array('pid'=>$gid,'vip_grade'=>2))->count();
			$level=3;//经理
			if($zhuguan_num>=$zg_num){
				$guser=M('user')->where(array('id'=>$gid))->find();
				if($guser['vip_grade']>=2){
					M('user')->where(array('id'=>$gid))->setField('vip_grade',$level);
				}
			}
		}
	}
 ...
 * $id 购买商品用户id
 */
function Addtgjl($pid,$gid,$ggid,$uid){
	$tgjl=array();
	$index=0;
	//直推会员奖励
	$zt_vip_grade=M('user')->where(array('userid'=>$pid))->getField('vip_grade');
	if($zt_vip_grade>=1){
		$zt_price=M('reward')->where('id=1')->getField('hy_price');
		$oldprice=M('store')->where('uid='.$pid)->getField('tuiguang_num');
		$tgjl[$index]['id']=$pid;
		$tgjl[$index]['price']=$zt_price;
		$tgjl[$index]['oldprice']=$oldprice;
		$index+=1;
	}
	//主管奖励
	$zg_vip_grade=M('user')->where(array('userid'=>$gid))->getField('vip_grade');
	if($zg_vip_grade>=2){
		$zg_price=M('reward')->where('id=1')->getField('zg_price');
		$oldprice=M('store')->where('uid='.$gid)->getField('tuiguang_num');
		$tgjl[$index]['id']=$gid;
		$tgjl[$index]['price']=$zg_price;
		$tgjl[$index]['oldprice']=$oldprice;
		$index+=1;
	}
	//经理奖励
	$jl_vip_grade=M('user')->where(array('userid'=>$ggid))->getField('vip_grade');
	if($jl_vip_grade>=3){
		$jl_price=M('reward')->where('id=1')->getField('jl_price');
		$oldprice=M('store')->where('uid='.$ggid)->getField('tuiguang_num');
		$tgjl[$index]['id']=$ggid;
		$tgjl[$index]['price']=$jl_price;
		$tgjl[$index]['oldprice']=$oldprice;
	}
	foreach ($tgjl as $key => $val){
		//生成奖励记录
		$data['pay_id']=$val['id'];
		$data['get_id']=$val['id'];
		$data['get_nums']=$val['price'];
		$data['get_time']=time();
		$data['get_type']=23;
		$data['now_nums']=$val['oldprice'];
		$data['now_nums_get']=$val['oldprice'];
		$data['is_release']=1;
		$res=M('tranmoney')->add($data);
		if($res){
			$tgjl_num=M('coins')->where(array('name'=>'MXC'))->getField('tgjl_num');
			if($tgjl_num-$val['price'] >= 0){
				M('store')->where(array('uid'=>$val['id']))->setInc('tuiguang_num',$val['price']);
				M('coins')->where(array('name'=>'MXC'))->setInc('yhcy_num',$val['price']);
				M('coins')->where(array('name'=>'MXC'))->setDec('tgjl_num',$val['price']);
			}
			return true;
		}else{
			return false;
