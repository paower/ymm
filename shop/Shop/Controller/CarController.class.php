...
class CarController extends CommonController
{
	...
	{
		$info = session('car');
		$selinfo = session('selCar');
		foreach ($selinfo as $k => $v) {
			# code...
        ...
        foreach ($car_arr as $key => $carList) {
	        foreach ($carList as $k => $v) {
	            ...
	            $where2['gid'] = $v['pid'];
	            $parameter = M("product_parameter")->where($where2)->count();
				if($parameter==0){
		            if(kuncun($v['pid'],$v['num'],$v['color'])===false){
					...
					$num = M("product_parameter")->where($where3)->getField('kucun');
					if($v['num']>$num){
						error_alert('商品'.$com_info['name'].'库存不足');
						return;
					}
	          ...
	          //  if(!in_array($com_info['id'],$sellerList[$com_info['shangjia']]['com_ids'])){
	                ...
	                $sellerList[$com_info['shangjia']]['type'][] = $v['type'];
        ...
        foreach ($sellerList as $key => $value) {
           ...
           foreach ($value['com_ids'] as $k => $v) {
			    $type = $sellerList[$shangjia]['type'][$k];
				...
				if($type!="请选择" && !empty($type)){
					$where2['gid'] = $product['id'];
					$where2['type'] = $type;
					$price = M('product_parameter')->where($where2)->getField('price');	
					$total_price = $total_price + $price * $num;
					$xiaofei_paynums = $xiaofei_paynums + $xiaofeibili * $num;
	                $tt=$total_price+$product['freight'];
					$total_price = $tt;
				}else{
				...
				//减少库存
				if($type!="请选择"){
					M("product_parameter")->where(array("gid" => $product['id'],"type"=>$type))->setDec("kucun", $num);
				}else{
	...
	public function Buynow()
	{
		...
		if($color!="请选择"){
			...
			$where['gid']=$pid;
			$where['type'] = $color;
			$parameter = M('product_parameter')->where($where)->find();
			if(!$pdetail || !$parameter){
				$this->error("该商品不存在");
			};
		}else{
			$pdetail = M("product_detail")->where(array("id"=>$pid))->find();
		...
		if ($car) {
			...
			if ($product) {
				...
				foreach ($product as $key => $prod) {
					if ($prod['pid'] == $pid && $prod['size'] == $size && $prod['color'] == $color && $jifentype == $prod['type']) {
						$product[$key]['num'] = $num;
						$tag = "ok";
					}
				}
