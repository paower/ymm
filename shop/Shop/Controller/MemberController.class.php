...

{
...

	{
		...
		//统计用户购买的套餐额度
		$user_set_meal = M('order o')->where(array('o.uid'=>$uid,'o.status'=>3,'o.order_sellerid'=>0,'buy_price'=>array('egt',6800)))->find();
		if($user_set_meal){
			$set_meal_num = 1;
		}else{
			$set_meal_num = 0;
		...
		//已售订单数量查询
		$sold_orders = M("order")->where(array("order_sellerid"=>$uid))->count();
		$this->assign('sold_orders',$sold_orders);
		...
		$dfk = M('order')->where("uid=$uid and status=0")->count();
		$dfh = M('order')->where("uid=$uid and status=1")->count();
		$dsh = M('order')->where("uid=$uid and status=2")->count();
		$ywc = M('order')->where("uid=$uid and status=3")->count();
		$this->assign('dfk',$dfk);
		$this->assign('dfh',$dfh);
		$this->assign('dsh',$dsh);
		$this->assign('ywc',$ywc);

		$this->assign('set_meal_num',$set_meal_num);
...

	{
		...
		// dump($orderinfo);die;
		$kd_name = $orderinfo[0]['kd_name'];
		$where['com'] = $kd_name;
		$kd_no = M('exp_com')->where($where)->getField('no');
		header('Content-type:text/html;charset=utf-8');
		$params = array(
		  'key' => 'b7f24237b91910ee72dc1fb9f38f100f', //您申请的快递appkey
		  'com' => $kd_no, //快递公司编码，可以通过$exp->getComs()获取支持的公司列表
		  'no'  => $orderinfo[0]["kd_no"] //快递编号
		);
		$exp = new ExpController($params['key']); //初始化类
		...
		if($result['error_code'] == 0){//查询成功
		  ...
		  $this->error("获取失败，原因：".$result['reason']);
		}
...

	{
		...
		if ($mine_jiefn < $txnums) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,您暂未拥有这么多MXC');
		...
		if ($txnums < $withconfigs['tixian_start']) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,最低起提MXC是' . $withconfigs['tixian_start'] . '');
...

	{
		...
		$nctx['order_sellerid'] = $uid;

		$member = M("order o");
		// ->join('ysk_order_detail od On o.order_id=od.order_id')

		$count = $member->where($nctx)->join('ysk_user u ON o.uid=u.userid')->count(1);// 查询满足要求的总记录数
		...
		// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
// ->join('ysk_order_detail od On o.order_id=od.order_id')
		$list = $member->where($nctx)->join('ysk_user u ON o.uid=u.userid')->order('order_id desc')->field('o.time,o.buy_price,u.username,o.order_id,o.uid')->limit($Page->firstRow . ',' . $Page->listRows)->select();

		foreach($list as $k => $v){
			$list[$k]['com_name'] = M('order_detail')->where(array('order_id'=>$v['order_id']))->getField('com_name');
		}
		$total_sum = $member->where($nctx)->join('ysk_user u ON o.uid=u.userid')->sum('buy_price');
...

	{
		...
		if ($res_change) {
			...
			$pay_money = M('user')->where("userid=$userid")->getField("pay_money");
			$pay_money+=$money;
			$data['pay_money']=$pay_money;
			M('user')->where("userid=$userid")->save($data);
			$info = M('order_detail')->where("order_id=$orderid")->field('com_id')->select();
			foreach ($info as $k => $v) {
				$com_id = $v['com_id'];
				$buy_num = M('product_detail')->where("id=$com_id")->getField('buy_num')+1;
				M('product_detail')->where("id=$com_id")->setField('buy_num',$buy_num);
			}
	...
	public function Mineorders(){
		...
		$orderList = M("order")->where(array("order_sellerid"=>$uid))->order('time desc')->select();
	...
	public function deliver(){
		...
		$res = M('order')->where(array('order_id'=>$id,"order_sellerid"=>$uid))->field('status')->find();
		$shop_cate = M('gerenshangpu')->where(array('userid'=>$uid))->getField('shop_cate');
		...
		if($res){
			$end_time = time()+$goods_end;
			$arr = array(
				"kd_name" => $express_name,
				"kd_no" => $express_order,
				"status" => 2,
				"deadline" => $end_time
			);
			...
			$res_deliver = M('order')->where(array('order_id'=>$id))->setfield($arr);
	...
	public function verify(){
		$uid = session('userid');
		$is_dailishang = M('user')->where('userid='.$uid)->getField('is_dailishang');
		...
		// if($set_meal_num == 0 && $is_dailishang==0){
			// echo '<meta charset="utf-8"><script>alert("购买6800元套餐以上的才能申请商家入驻");location.href="'.U('member/mine').'"</script>';
			// die;
		// }
	...
	/*		var_dump($userid);die;*/
			...
			M('user')->where(array('userid'=>$userid))->save($arr);
			...
			$d_type=M('product_cate')->field('id,name,type')->order('sort desc')->select();
	...
	public function agreem(){
		$detail = M('wenzhang')->where(array('id'=>17))->find();
		// dump($detail);
