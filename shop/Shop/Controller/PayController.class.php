...
/*echo $oid;die;*/
		...
		//判断顶单商品支付方式
		$goodinfo = M('order_detail')->where(array('order_id' => $all_oid))->field('com_id')->select();
		$goods_id = array();
		foreach($goodinfo as $vo){
			$goods_id[] = $vo['com_id'];
		}
		$goods_id = implode(',',$goods_id);
		$zhifupay = M('product_detail')->where(array('id'=>array('in',$goods_id)))->getField('zhifu_pay');
		$zhifupay = explode(',',$zhifupay);
		//支付权限
		$pay_shangjiajm = M('user')->where("userid = $uid")->getField('pay_shangjiajm');
		$pay_xiaofeijm = M('user')->where("userid = $uid")->getField('pay_xiaofeijm');
		$pay_duihuan = M('user')->where("userid = $uid")->getField('pay_duihuan');
		$this->assign("pay_shangjiajm",$pay_shangjiajm);
		$this->assign("pay_xiaofeijm",$pay_xiaofeijm);
		$this->assign("pay_duihuan",$pay_duihuan);
		// dump($zhifupay);die;
		$this->assign('zhifupay',$zhifupay);
		//该订单是否有打款图
		//查询农联金额
		$cangku_num = M("store")->where(array("uid" => $uid))->getField("cangku_num");
		$cangku2_num = M("store")->where(array("uid" => $uid))->getField("cangku2_num");
		$recharge_num = M("store")->where(array("uid" => $uid))->getField("recharge_num");
		$this->assign("cangku_num", $cangku_num);
		$this->assign("cangku2_num", $cangku2_num);
	...
	public function pays()
	{
		...
		if ($type == 0) {
			...
			$neednums = M('store')->where(array('uid' => $uid))->getField('recharge_num');
			$pay_type = 2;
		}elseif($type == 2){
			$neednums = M('store')->where(array('uid' => $uid))->getField('cangku2_num');
			$pay_type = 3;
		}
		...
		if($type == 3){
			...
			$notify_url = "http://ymm.com/Login/wxbuyresult"; //回调地址
		...
		if ($neednums < $pay_money) {
			...
			foreach($com_id as $vo){
					//赠送积分
					if($pay_type==2){
						$goods_num = M('product_detail')->where(array('id'=>$vo['com_id']))->getField('jifen_nums');
						$goods_num = $goods_num * $vo['com_num'];
						$jifen_nums += $goods_num;
		    ...
		    if($type==0){
					...
					$dec_cang = M('store')->where(array('uid'=>$uid))->getField('cangku_num');
					//  写入余额减少记录
					$traInfo = M('tranmoney');
					$res_dec_tran['pay_id'] = $uid;
					$res_dec_tran['get_nums'] = -$pay_money;
					$res_dec_tran['createtime'] = time();
					$res_dec_tran['get_type'] = 4;
					$res_dec_tran['now_nums'] = $dec_cang;
					$res_dec_tran['now_nums_get'] = $dec_cang;
					$res_dec_tran['is_release'] = 1;
					$traInfo->add($res_dec_tran);

		    }elseif($type==1){
					$res_dec = M('store')->where(array('uid' => $uid))->setDec('recharge_num', $pay_money);
					
					$dec_cang = M('store')->where(array('uid'=>$ud))->getField('recharge_num');
					//  写入充值余额减少记录
					$traInfo = M('tranmoney');
					$res_dec_tran['pay_id'] = $uid;
					$res_dec_tran['get_nums'] = -$pay_money;
					$res_dec_tran['createtime'] = time();
					$res_dec_tran['get_type'] = 37;
					$res_dec_tran['now_nums'] = $dec_cang;
					$res_dec_tran['now_nums_get'] = $dec_cang;
					$res_dec_tran['is_release'] = 1;
					$traInfo->add($res_dec_tran);
				}elseif($type==2){
					$res_dec = M('store')->where(array('uid'=>$uid))->setDec('cangku2_num',$pay_money);

					$dec_cang = M('store')->where(array('uid'=>$uid))->getField('cangku2_num');
					//  写入Eta余额减少记录
					$traInfo = M('tranmoney');
					$res_dec_tran['pay_id'] = $uid;
					$res_dec_tran['get_nums'] = -$pay_money;
					$res_dec_tran['createtime'] = time();
					$res_dec_tran['get_type'] = 38;
					$res_dec_tran['now_nums'] = $dec_cang;
					$res_dec_tran['now_nums_get'] = $dec_cang;
					$res_dec_tran['is_release'] = 1;
					$traInfo->add($res_dec_tran);
			...
			if($jifen_nums&&$jifen_nums > 0 && $pay_type==2){

				//查询当前积分
				$jifen = M('store')->where(array('uid' => $uid))->getField('fengmi_num');
				//增加消费的积分
				$yin_jifen = $jifen_nums;
				...
				$data['get_type'] = 24;
				$data['now_nums'] = $jifen + $yin_jifen;
				$data['now_nums_get'] = $jifen + $yin_jifen;
				$data['is_release'] = 1;
				$traInfo->add($data);
			}

			// if($return_balance&&$return_balance > 0 && $pay_type == 2){
			...
			if ($res_dec) {
				...
				//判断是否为升级会员商品
				$is_teshu=M('product_detail')->where(array('id'=>$com_id))->getField('is_teshu');		
				if($is_teshu==1){
					$vip_grade=M('user')->where('userid='.$uid)->getField('vip_grade');
					if($vip_grade==0){
						M('user')->where('userid='.$uid)->setField('vip_grade',1);
					}
					
					M('order')->where(array('order_id' => $oid))->setField('status', 3);
					
					$pid=M('user')->where('userid='.$uid)->getField('pid');
					$p_info=M('user')->where(array('userid'=>$pid))->field('userid,pid,gid,username,account,mobile,path,deep')->find();
				...
				M()->commit();
				success_alert('支付成功', U('Home/index'));
			}else{
				M()->rollback();
				error_alert('支付失败');
			}
